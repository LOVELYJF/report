...
public class BddpController {
	...
	public R getJSONMapContent(String mappath) {
		FileReader fileReader = new FileReader(ServerUtil.getDataPath(relativePath, bddppath, BddpConstant.MAPFILE_PATH) + mappath);
		String result = fileReader.readString();
	...
	public R getBddpFolders() {
		List list = getFoldersList(new File(ServerUtil.getDataPath(relativePath, bddppath, BddpConstant.BDDPFILE_PATH)));
		return R.ok().put("res", list);
	}
	...
	public R getJSONFoldersContentById(String id) {
		String path = ServerUtil.getDataPath(relativePath, bddppath, BddpConstant.BDDPFILE_PATH);
		List<Map<String, String>> list = getFoldersList(new File(ServerUtil.getDataPath(relativePath, bddppath, BddpConstant.BDDPFILE_PATH)));
	...
	public R getBddpMapFilesTree() {
		...
		getFilesTree(new File(ServerUtil.getDataPath(relativePath, bddppath, BddpConstant.MAPFILE_PATH)), "0", jsonArray);
			...
			if (box.containsKey("parts")) {
				...
				if (parts.containsKey("backgroundImage")) {
					...
					if (backgroundImage.indexOf(fromId) > -1) {
						...
						FileUtil.copy(new File(bddpPath + fromId + File.separator + paths[1]), new File(diyTagsPath + id + File.separator + paths[1]), true);
					}
				}
